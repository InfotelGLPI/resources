...
class PluginResourcesResourceHabilitation extends CommonDBTM {
   ...
   function addResourceHabilitation($params) {
      ...
      foreach ($params as $key => $val) {
         if (is_array($val)
             && $habilitation_level->getFromDBByCrit(["name = '" . str_replace('_', ' ',$key) . "' AND number"])) {
            foreach ($val as $v) {
               $this->addResourceHabilitationInDb($v, $params);
            }
         } else if ($habilitation_level->getFromDBByCrit(["name = '" . str_replace('_', ' ',$key) . "' AND !number"])) {
   ...
   function checkRequiredFields($params = []) {
      ...
      foreach ($levels as $level) {
         if (!isset($params[str_replace(' ', '_', $level['name'])])
             || (isset($params[str_replace(' ', '_', $level['name'])]) && empty($params[str_replace(' ', '_', $level['name'])]))) {
            return false;
